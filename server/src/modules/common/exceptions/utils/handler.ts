...
export type ErrorHandler = (error: DatabaseError) => { message: string; constraint: string | null };

export const PostgresErrorHandlers: Record<string, ErrorHandler> = {
  "23505": (error) => ({
 ...
 */
export function extranctDbError(error: DrizzleQueryError): {
  message: string;
